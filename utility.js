...
function initialize93Board() {
    ...
    for (let node = 1; node <= nnodes; node++)
        nodes.push({
            x: 25 + coordinates[(node - 1) * 2] * (canvasSize - 50),
            y: canvasSize - 25 - coordinates[(node - 1) * 2 + 1] * (canvasSize - 50),
            player: 0
        });
    ...
    for (let node = 1; node <= nnodes; node++) {
        ...
        for (let neighbour = 1; neighbour <= 6; neighbour++)
            if (neighbours[(node - 1) * 6 + neighbour - 1] !== 0)
                adjacencyList[node - 1].push(neighbours[(node - 1) * 6 + neighbour - 1] - 1);
    }
}

function drawBoard(p5) {
    p5.stroke(p5.color(84, 8, 4));
    p5.strokeWeight(2);
    for (let node = 0; node < nnodes; node++) {
        for (let nneighbour = 0; nneighbour < adjacencyList[node].length; nneighbour++)
            p5.line(nodes[node].x, nodes[node].y, nodes[adjacencyList[node][nneighbour]].x, nodes[adjacencyList[node][nneighbour]].y);
    }
    for (let node = 0; node < nnodes; node++) {
        if (nodes[node].player === 0) {
            p5.fill(p5.color(84, 8, 4));
            p5.ellipse(nodes[node].x, nodes[node].y, 15, 15);
        } else {
            p5.fill(nodes[node].player === 1 ? p5.color(239, 239, 239) : p5.color(4, 2, 40));
            p5.ellipse(nodes[node].x, nodes[node].y, 30, 30);
        }
    }
}
...
function startGame() {
    gameStarted = true;
    game = new app.Game("93Board", nnodes, adjacencyList);
...
let local_p5 = new p5(function(p5) {
    ...
    p5.draw = function() {
        p5.background(p5.color(248, 112, 96));
        drawBoard(p5);
    }
